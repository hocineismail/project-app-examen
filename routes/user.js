  ...
  user.post('/reset/:token', function(req, res) {
    ...
    async.series([function(callback){
        ...
        Phase.find({},function(err,phase){
            ...
            phases = phase;
            callback(null,phases);
>>>>>>> 77a6119568086824c05d358b1e50089cbc66a530
        })
      },
      function(token, user, done) {
        var smtpTransport = nodemailer.createTransport({
...
user.get('/static', ensureAuthenticated, function(req, res) {
  ...
  async.series(
    [
      ...
      function(callback) {
        Question.countDocuments({ IsValidFinal: true }, function(
          err,
          validFinal
        ) {
          if (err) return callback(err)
    ...
    function(err) {
      res.render('static', {
        ...
        CountValidOne: CountValidOne,
        CountValidTwo: CountValidTwo,
        CountValidFinal: CountValidFinal
      })
    }
  )
})

user.get('/admin/deletephase/:_id', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.post('/admin/updatephase/:_id', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.post('/admin/updatelevel/:_id', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.get('/admin/deletelevel/:_id', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.get('/admin/:module/deleteexam/:id', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.post('/admin/updatesemster/:_id', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.get('/admin/:level/deletesemster/:_id', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.post('/admin/:level/:_id/updatemodule', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
<<<<<<< HEAD
user.post('/admin/:level/:_id/updatesemster', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.get('/admin/deletesemster/:_id', ensureAuthenticated, function(
  req,
  res,
  next
) {
...
user.get('/admin/:level/deletemodule/:_id', ensureAuthenticated, function(
  req,
  res,
  next
) {
